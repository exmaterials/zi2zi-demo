...
import argparse
import pathlib
from font2img_single import generate_font_image
...
def get_target_chara(src_font_idx, chara):
    data_root = data_root = pathlib.Path('./fonts/gradio_target')
    ...
    return generate_font_image(all_image_paths[src_font_idx], f'{chara}', img_size= 256, chara_size= 192, x_offset=20, y_offset=-10)
...
def to_infer(char, label):
    ...
    parser.add_argument('--resume', type=int, default=80000)
    parser.add_argument('--src_font', default = './fonts/source/NotoSansTC-Regular.ttf')
    ...
    parser.add_argument('--result_folder', default = 'gradio')
    ...
    with torch.no_grad():
        ...
        # target_img = generate_font_image(args.src_font,ch, 256, 172,40,0).convert('L')
